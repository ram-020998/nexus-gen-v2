...
def main():
    ...
    add_paragraph(doc, 'NexusGen is an AI-powered platform that transforms document processing for low-code developers. Using Amazon Q CLI agents and AWS Bedrock, it automates requirements analysis, design validation, and documentation generation—reducing manual effort by 90% while maintaining enterprise-quality standards.')
    ...
    rows = [
        ['Context Awareness', 'Limited to individual knowledge', 'No organizational context', 'Bedrock KB with org patterns'],
    ...
    add_paragraph(doc, 'NexusGen transforms low-code development by automating the most time-consuming aspects of software design while maintaining enterprise quality standards. With 90%+ time savings in document processing and AI-powered quality assurance, it\'s essential infrastructure for scaling low-code teams.')
    ...
    set_font(run, "Tahoma", 11)
    run.bold = True
    run2 = para.add_run('Enable low-code developers to deliver architect-quality work at developer speed, accelerating project delivery by 40-60% while building organizational knowledge assets.')
