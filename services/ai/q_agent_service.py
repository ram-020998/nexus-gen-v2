...
class QAgentService(BaseService):
    """Handle Q CLI agent operations with dependency injection"""

    def __init__(self, container: Optional[DependencyContainer] = None):
        """
        Initialize Q Agent service
    ...
    def process_breakdown(self, request_id: int, file_content: str, bedrock_context: dict) -> dict:
        ...
        Args:
            ...
            file_content: Content to break down
            bedrock_context: Context from Bedrock RAG
    ...
    def process_verification(self, request_id: int, design_content: str, bedrock_context: dict) -> dict:
        ...
        Args:
            request_id: Request ID for tracking
            design_content: Design content to verify
            bedrock_context: Context from Bedrock RAG
    ...
    def process_creation(self, request_id: int, acceptance_criteria: str, bedrock_context: dict) -> dict:
        ...
        Args:
            request_id: Request ID for tracking
            acceptance_criteria: Acceptance criteria for design
            bedrock_context: Context from Bedrock RAG
    ...
    def process_conversion(self, request_id: int, maria_sql: str) -> str:
        ...
        Args:
            request_id: Request ID for tracking
            maria_sql: MariaDB SQL to convert
            
        Returns:
            Converted Oracle SQL string
        """
        try:
    ...
    def _execute_q_agent(self, agent_name: str, prompt: str) -> subprocess.CompletedProcess:
        ...
        Args:
            agent_name: Name of the Q agent to execute
    ...
    def _extract_sql_from_output(self, result: subprocess.CompletedProcess) -> str:
        ...
        Returns:
            Extracted SQL string or None if extraction fails
        """
        try:
            # Q agent outputs to stdout
            output = result.stdout
...
-- 4. Sequence creation for AUTO_INCREMENT columns"""
    ...
    def _load_application_artifacts(self) -> str:
        ...
        try:
            from config import Config
            artifacts_dir = Path(Config.BASE_DIR) / "applicationArtifacts" / "Source Selection"
...
"""
            ...
            # Write to temp file
            temp_file = Path(Config.BASE_DIR) / "temp_artifacts_summary.txt"
