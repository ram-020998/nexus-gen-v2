...
class PackageService(BaseService):
    ...
    def _initialize_dependencies(self):
        ...
        self.appian_object_repo = self._get_repository(AppianObjectRepository)
    ...
    def create_package_from_blueprint(
        ...
        try:
            ...
            # Create Package record using repository
    ...
    def get_object_by_uuid(
        ...
        """
        return self.appian_object_repo.get_by_uuid(package_id, uuid)
    ...
    def get_objects_by_type(
        ...
        """
        return self.appian_object_repo.get_by_type(package_id, object_type)
    ...
    def extract_and_create_objects(
        ...
        try:
            ...
            for uuid, obj_data in object_lookup.items():
                ...
                # Handle both 'object_type' and 'type' keys for compatibility
                object_type = obj_data.get('object_type') or obj_data.get('type', '')
    ...
    def extract_and_create_process_models(
        ...
        try:
            ...
            for uuid, obj_data in object_lookup.items():
                # Only process Process Model objects (check both 'object_type' and 'type')
                obj_type = obj_data.get('object_type') or obj_data.get('type', '')
                if obj_type != 'Process Model':
                    continue

                # Process models can have nodes/flows directly or in process_model_data
                pm_data = obj_data.get('process_model_data', {})
                nodes = obj_data.get('nodes', pm_data.get('nodes', []))
                flows = obj_data.get('flows', pm_data.get('flows', []))
                ...
                appian_obj = self.appian_object_repo.get_by_uuid(
                    package.id,
                    uuid
                )
                ...
                for node in nodes:
                    # Use 'uuid' field from node, or 'id' for compatibility
                ...
                for flow in flows:
                    # Flows use 'from_node_uuid' and 'to_node_uuid' keys, or 'from'/'to' for compatibility
                    from_node_uuid = flow.get('from_node_uuid') or flow.get('from', '')
                    to_node_uuid = flow.get('to_node_uuid') or flow.get('to', '')
